...
//Rendezzük az elõzõ feladat listáját monogram / életkor / magasság szerinti növekvõ sorrendbe!
...
//torli azokat akik 2000 utan szulettek
...
ListaElem* Rendez(ListaElem* Eleje) {
    ListaElem* eakt = Eleje;
    ListaElem* rendezet = nullptr;
    while (eakt) {
        ListaElem* eakov = eakt->kov;
        ListaElem* rendelozo = nullptr;
        ListaElem* rendakt = rendezet;
        while (rendakt && rendakt->kor < eakt->kor) {
            rendelozo = rendakt;
            rendakt = rendakt->kov;
        }
        if (rendelozo) {
...
int main()
{
    ...
    Eleje = Keszit(Eleje);
    Kiir(Eleje);
    ListaElem* rendezett = Rendez(Eleje);
    Kiir(rendezett);
